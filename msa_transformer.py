...
class MSATransformer(nn.Module):
    ...
    def forward(self, tokens, repr_layers=[], need_head_weights=False, return_contacts=False):
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            # directly sum col_attn_weights to reduce memory cost
            # col_attn_weights: B x H x C x R x R
            if need_head_weights:
                x, col_attn, _ = x
                if layer_idx == 0:
                    col_attn_weights = symmetrize(col_attn.permute(2, 0, 1, 3, 4))
                else:
                    col_attn_weights += symmetrize(col_attn.permute(2, 0, 1, 3, 4))
        ...
        if need_head_weights:
            ...
            # B x H x C x R x R -> B x C x R x R
            result["col_attentions"] = col_attn_weights.sum(1)
