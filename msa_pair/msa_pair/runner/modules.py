...
class Attention(hk.Module):
  ...
  def __call__(self, q_data, m_data, bias, nonbatched_bias=None):
    ...
    # print("init_q: ", q_data, "k", k, "logits: ", logits)
    ...
    # logits: [Residues, head, num_msa, num_msa]
...
class MSARowAttentionWithPairBias(hk.Module):
  ...
  def __call__(self,
    ...
    msa_act, col_attn = mapping.inference_subbatch(
        attn_mod,
        self.global_config.subbatch_size,
...
class MSAColumnAttention(hk.Module):
  ...
  def __call__(self,
    ...
    attn_mod = Attention(
        c, self.global_config, msa_act.shape[-1])
    # print("self.global_config.subbatch_size,", self.global_config.subbatch_size)
    self.global_config.subbatch_size = 256
    msa_act, col_attns = mapping.inference_subbatch(
        attn_mod,
        self.global_config.subbatch_size,
        batched_args=[msa_act, msa_act, bias],
        nonbatched_args=[],
...
class TriangleAttention(hk.Module):
  ...
  def __call__(self, pair_act, pair_mask, is_training=False):
    ...
    pair_act, col_attn = mapping.inference_subbatch(
        attn_mod,
        self.global_config.subbatch_size,
...
class EvoformerIteration(hk.Module):
  ...
  def __call__(self, activations, masks, is_training=True, safe_key=None):
    ...
    msa_act = dropout_wrapper_fn(
        ...
        safe_key=next(sub_keys),
        pair_act=pair_act)
    # print("self.is_extra_msa:", self.is_extra_msa)
    ...
    #     msa_mask,
    #     safe_key=next(sub_keys))
    msa_act, col_attn = attn_mod(msa_act, msa_mask)
    print(msa_act, col_attn)
    exit()
...
class TemplateEmbedding(hk.Module):
  ...
  def __call__(self, query_embedding, template_batch, mask_2d, is_training):
    ...
    embedding, col_attn = mapping.inference_subbatch(
