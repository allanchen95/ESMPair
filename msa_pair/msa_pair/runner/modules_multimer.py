...
from msa_pair.runner import modules
...
#   print(id_print(index_order))
#   call(lambda x: print(f"There are {x} true bools"), index_order[:6])
#   exit()
  sel_idx = extra_idx = index_order
#   extra_idx = index_order[index_order.shape[0]:]
#   print(extra_idx)
...
class AlphaFold(hk.Module):
  ...
  def __call__(
    ...
    self.config.num_recycle = 0
    # print("recycle: cuurr", self.config.num_recycle)
...
class EmbeddingsAndEvoformer(hk.Module):
  ...
  def __call__(self, batch, is_training, safe_key=None):
    ...
    # print("config: ", c.num_msa, c.masked_msa)
    # print(sample_key)
    # print(batch)
    # exit()

    # without msa sampling
